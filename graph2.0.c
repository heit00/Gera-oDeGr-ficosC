...
bool tryXY(int x, int y){// fun??o de valida??o de posicionamento e aproveitamento l?gico para a cria??o do grafico
...
void setConsoleSize(int cols, int rows) {
    ...
    // Pega o tamanho m?ximo da janela para este console (considerando resolu??o/fonte)
    COORD maxWindowSize = GetLargestConsoleWindowSize(hConsole);

    // Ajusta o tamanho da janela para o menor entre o que deseja e o m?ximo poss?vel
...
void setConsoleMaxSize() {
    ...
    // Pega o tamanho m?ximo da janela que cabe na tela (em caracteres)
    COORD maxSize = GetLargestConsoleWindowSize(hConsole);
    if (maxSize.X == 0 || maxSize.Y == 0) {
        printf("Erro ao obter tamanho m?ximo da janela\n");
        return;
    }

    // Primeiro reduz a janela temporariamente para evitar conflito na mudan?a do buffer
    SMALL_RECT tmpWindow = {0, 0, 1, 1};
    SetConsoleWindowInfo(hConsole, TRUE, &tmpWindow);

    // Ajusta o tamanho do buffer para o m?ximo permitido
...
void drawGraphTemplate(){// fun??o para desenhar o gr?fico
	int i;
...
void drawFunctionTemplate(int minDomain, int maxDomain, float (*f)(int), float tam_escala){//desenha a fun??o definida em fucntion(x: int):void e complemente o gr?fico de drawGraphTemplate():void
	if(tam_escala <= 0){
	...
	for(i = minDomain; i < maxDomain; i++){
		...
		if(tryXY(x,y)){
			xyCursor(x,y);
			printf("*");
			ajustPoints(temp_x,temp_x2,f, tam_escala);//ajustar exibi??o de valores x
		}		
	}
	textcolor(WHITE);
}
